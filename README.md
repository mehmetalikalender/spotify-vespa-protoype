...
Bu repository, Vespa.ai vektör arama motoru, Django (Backend) ve SeaweedFS (Dağıtık Dosya Sistemi) kullanılarak geliştirilmiş kavramsal bir müzik öneri sistemi prototipidir.
...
Proje içerisinde hazır bir db.sqlite3 veritabanı bulunmaktadır.
Bu veritabanındaki örnek şarkıların vektörleştirilip (mock embedding) Vespa'nın belleğine (HNSW graph) yüklenmesi için Django konteynerinin içine aşağıdaki komutu gönderin:
...
http://localhost:8000/api/music/recommendations/?strategy=similarity_ranking
🎯 Benzerlik + Popülerlik Odaklı Hibrit Sıralama (Hybrid Ranking)
http://localhost:8000/api/music/recommendations/?strategy=hybrid_ranking
🛠️ Tasarım Kararları ve Bilinmesi Gerekenler
📦 Veritabanı (SQLite)

Testlerin hızlı yapılabilmesi amacıyla adet örnek şarkı verisi barındıran db.sqlite3 dosyası kasıtlı olarak repository'e dahil edilmiştir.

🎵 Müzik Dosyaları ve SeaweedFS

Mimaride medya dosyalarının veritabanını şişirmemesi adına SeaweedFS kullanılmıştır.

Ancak repository boyutunu optimize etmek için fiziksel MP3 dosyaları (seaweedfs_data klasörü) repoya eklenmemiştir.

API’den dönen JSON yanıtlarındaki url alanları mimarinin doğru çalıştığını gösterir.
Ancak bu linklere tıklandığında fiziksel dosyalar projede olmadığı için 404 Not Found dönmesi beklenen bir prototip davranışıdır.

🧠 Mock Embedding

Şarkıların müzikal özellikleri 3 boyutlu vektörler olarak simüle edilmiştir.

Test sonuçlarının deterministik ve tutarlı olması adına vektörler:
